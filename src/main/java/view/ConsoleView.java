...
public class ConsoleView implements View{
    ...
    public SelectRequestDTO selectMode() throws IOException {
        System.out.println("\nQ. 모드를 선택해주세요.\n" +
                "1. 일반 모드\n" +
                "2. 테스트 모드\n");
        return SelectRequestDTO.builder().number(Integer.parseInt(br.readLine())).build();
    }
    ...
    public CreateBookRequestDTO addBook() throws IOException {
        ...
        return CreateBookRequestDTO.builder().title(title).author(author).pageNum(pageNum).build();
    }
    ...
    public void listBooks(List<Book> bookList) {
        ...
        bookList.stream().forEach(book -> {
            System.out.println("도서번호 : " + book.getId() +
                    ...
                    "\n상태 : " + book.getBookStatusType().getDescription() +
                    "\n\n------------------------------\n");
        });
    ...
    public CreateBookRequestDTO searchBook() throws IOException {
        ...
        String title = br.readLine();
        return CreateBookRequestDTO.builder().title(title).build();
    }
