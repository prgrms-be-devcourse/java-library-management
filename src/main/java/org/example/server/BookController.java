...
public enum BookController {
    REGISTER(request -> {
        BookService.register(request.requestData.requestBookDto);
...
//        return "\n[System] 존재하는 도서가 없습니다.\n";
    ...
    SEARCH_BY_NAME(request -> {
        return BookService.searchByName(request.requestData.bookName) + "\n[System] 검색된 도서 끝\n";
//        return "\n[System] 존재하는 도서가 없습니다.\n";
    }),
    BORROW(request -> {
        /*
         ...
         */
        BookService.borrow(request.requestData.bookId);
...
//        return "\n[System] 존재하지 않는 도서입니다.\n";
    ...
    RETURN(request -> {
         ...
         */
        BookService.restore(request.requestData.bookId);
...
//        return "\n[System] 존재하지 않는 도서입니다.\n";
    ...
    LOST(request -> {
         ...
         */
        BookService.lost(request.requestData.bookId);
        return "\n[System] 도서가 분실 처리 되었습니다.\n";
//        return "\n[System] [System] 이미 분실 처리된 도서입니다.\n";
//        return "\n[System] 존재하지 않는 도서입니다.\n";
    }),
    DELETE(request -> {
        BookService.delete(request.requestData.bookId);
        return "\n[System] 도서가 삭제 처리 되었습니다.\n";
//        return "\n[System] 존재하지 않는 도서번호 입니다.\n";
//        return "\n[System] 해당 도서는 정리중입니다.\n";
    });
    public final Function<Request, String> mappingFunction;

    BookController(Function<Request, String> mappingFunction) {
        this.mappingFunction = mappingFunction;
    }

    public String mapping(Request request) {
